...
function push_helm_chart() {
  ...
  mkdir -p "${sync_dir}"
  set -ex
  helm package helm/tailing-sidecar-operator --dependency-update --version="${version}" --app-version="${version}" --destination "${sync_dir}"

  git fetch origin gh-pages
  git reset --hard HEAD
  git checkout gh-pages

  helm repo index --url "https://sumologic.github.io/tailing-sidecar${chart_dir:1}/" --merge "${chart_dir}/index.yaml" "${sync_dir}"
