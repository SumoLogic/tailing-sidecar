...
[[ $(kubectl logs statefulset-with-annotations-0 tailing-sidecar-0 -n tailing-sidecar-system --tail 5 | grep example | wc -l) -ne 5 ]] && exit 1
...
[[ $(kubectl logs statefulset-with-annotations-0 tailing-sidecar-2 -n tailing-sidecar-system --tail 5 | grep example | wc -l) -ne 5 ]] && exit 1
...
[[ $(kubectl logs ${DEPLOYMENT_POD_NAME} tailing-sidecar-2 -n tailing-sidecar-system --tail 5 | grep example | wc -l) -ne 5 ]] && exit 1
...
[[ $(kubectl logs ${DAEMONSET_POD_NAME} tailing-sidecar-2 -n tailing-sidecar-system --tail 5 | grep example | wc -l) -ne 5 ]] && exit 1
