...
# Test Pod with configuration in CR
readonly POD_WITH_CR="pod-with-tailing-sidecar-config"
wait_for_pod ${NAMESPACE} ${POD_WITH_CR} ${TIME}
[[ $(kubectl logs ${POD_WITH_CR} sidecar-0 -n ${NAMESPACE} --tail 5 | grep example | wc -l) -ne 5 ]] && exit 1
[[ $(kubectl logs ${POD_WITH_CR} sidecar-1 -n ${NAMESPACE} --tail 5 | grep example | wc -l) -ne 5 ]] && exit 1
